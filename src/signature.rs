...
impl ThresholdSignature {
    ...
    pub fn from_bytes(bytes: [u8; 64]) -> Result<ThresholdSignature, ()> {
        ...
        let R = CompressedEdwardsY(array).decompress().ok_or(())?;
...
//     and its algorithms?
#[derive(Debug)]
struct SignerRs(pub(crate) HashMap<[u8; 4], EdwardsPoint>);
...
#[derive(Debug)]
pub(crate) struct IndividualPublicKeys(pub(crate) HashMap<[u8; 4], EdwardsPoint>);
...
impl ThresholdSignature {
    ...
    pub fn verify(&self, group_key: &GroupKey, message_hash: &[u8; 64]) -> Result<(), ()> {
        ...
        let R_prime =
            EdwardsPoint::vartime_double_scalar_mul_basepoint(&c_prime, &-group_key.0, &self.z);
...
mod test {
    ...
    fn aggregator_get_signers() {
        ...
        let mut aggregator = SignatureAggregator::new(
            params,
            GroupKey(EdwardsPoint::identity()),
            &context[..],
            &message[..],
        );
