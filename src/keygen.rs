...
fn generate_shares(
    ...
    for j in 0..t {
        commitment
            .0
            .push(&coefficients.0[j] * &ED25519_BASEPOINT_TABLE);
    }
...
impl IndividualPublicKey {
    ...
    #[allow(unused)]
    pub fn verify(&self, parameters: &Parameters, commitments: &[EdwardsPoint]) -> Result<(), ()> {
...
mod test {
    ...
    fn secret_share_from_one_coefficients() {
        ...
        for i in 0..5 {
            commitments
                .0
                .push(&ED25519_BASEPOINT_TABLE * &coefficients.0[i]);
        }
    ...
    fn single_party_keygen() {
        ...
        assert!(
            p1_group_key.0.compress() == (&p1_secret_key.key * &ED25519_BASEPOINT_TABLE).compress()
        );
    }
